//code from http://rosettacode.org/wiki/Reduced_row_echelon_form#C.2B.2B
...
// specialization of the matrix traits for ublas (dense double) matrix
template<>
 struct matrix_traits<ublas_matrix>
{
  typedef std::size_t index_type;
  typedef double value_type;
  
  static index_type min_row(ublas_matrix const& A)
  { return 0; }
  
  static index_type max_row(ublas_matrix const& A)
  { return A.size1()-1; }
  
  static index_type min_column(ublas_matrix const& A)
  { return 0; }
  
  static index_type max_column(ublas_matrix const& A)
  { return A.size2()-1; }
  
  static value_type& element(ublas_matrix& A,
                             index_type i, index_type k)
  { return A(i,k); }
  
  static value_type element(ublas_matrix const& A,
                            index_type i, index_type k)
  { return A(i,k); }
};
