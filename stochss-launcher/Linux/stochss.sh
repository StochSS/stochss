...
then
	...
	docker start stochsscontainer1_7 >> $DIR/.dockerlog 2>&1 || { docker run -d -p 8080:8080 -p 8000:8000 --name=stochsscontainer1_7 stochss/stochss-launcher:1.7 sh -c "cd stochss-master; ./run.ubuntu.sh -t $token --yy" >> $DIR/.dockerlog && echo "Starting StochSS 1.7 for the first time." && echo "To view Logs, run \"docker logs -f stochsscontainer\" from another terminal"; } ||	{ echo "Failed to start server."; clean_up; exit; }
