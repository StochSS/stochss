...
class RunWorkflowAPIHandler(APIHandler):
    ...
    async def get(self):
        ...
        exec_cmd = ["/stochss/stochss/util/run_workflow.py", "{}".format(model_path), "{0}".format(workflow_path), "{0}".format(wkfl_type) ] # Script commands
...
class SaveWorkflowAPIHandler(APIHandler):
    ...
    async def get(self):
        ...
        # exec_cmd = [ "/stochss/stochss/util/run_workflow.py", "{0}".format(model_path), "{0}".format(workflow_path), "{0}".format(wkfl_type) ] # Script commands
        # opt_type = list(map(lambda el: "-" + el, list(opt_type))) # format the opt_type for argparse
        # exec_cmd.extend(opt_type) # Add opt_type to exec_cmd
        # log.debug("Exec command sent to the subprocess: {0}".format(exec_cmd))
