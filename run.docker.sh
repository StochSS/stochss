...
#sudo docker run -it -p 8080:8080 -p 8000:8000 -p 9999:9999 --name=stochsscontainer1_8_dev --volume `pwd`:/stochss-master --workdir /stochss-master aviralcse/stochss_qsub bash -c "apt-get install -yy docker ; pip install docker-py ; ./run.ubuntu.sh --yy -a 0.0.0.0"
