...
public final class ScoredDocument extends Document implements Serializable {
 ...
 private final Cursor cursor;
  ...
  /**
   * The list of scores assigned during sort evaluation. Each sort
   * dimension is included in this list. Positive scores are used
   * for ascending sorts; negative scores are used for descending.
   *
   * @return the list of scores assigned during sort evaluation
   */
  ...
  public String toString() {
    return String.format(
        "ScoredDocument(documentId=%s, fields=%s%s, rank=%d, " +
        "scores=%s, expressions=%s%s)",
        getId(),
        Util.iterableToString(getFields(), MAX_FIELDS_TO_STRING),
        Util.fieldToString("locale", getLocale()),
        getRank(),
        Util.iterableToString(scores, 0),
        Util.iterableToString(expressions, 0),
